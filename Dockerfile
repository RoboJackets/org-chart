...
RUN set -eux && \
    ...
    DJANGO_SETTINGS_MODULE=orgchart.settings.development DJANGO_ALLOWED_HOSTS=127.0.0.1 OIDC_RP_CLIENT_ID=build OIDC_RP_CLIENT_SECRET=build ./manage.py collectstatic --no-input && \
    cd /app/static/ && \
